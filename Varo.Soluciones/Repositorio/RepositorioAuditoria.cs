    ...
    public class RepositorioAuditoria : RepositorioBase, IRepositorioAuditoria
    {
        public IList<CalificacionAuditoria> Obtener(Guid idSolucion)
        {
            ...
            using (DbCommand command =
                this.BaseDeDatos.GetStoredProcCommand("uspAuditoriaSolucionSelectPorId"))
            {
                ...
                using (IDataReader dataReader = this.BaseDeDatos.ExecuteReader(command))
                {
                    ...
                    while (dataReader.Read())
                    {
                        ...
                        calificacionAuditoria.IdSolucion = dataReader.ToGuid("IdSolucion");
        ...
        public CalificacionAuditoria ObtenerUltimaCalificacion(Guid idSolucion)
        {
            ...
            using (DbCommand command =
                this.BaseDeDatos.GetStoredProcCommand("uspAuditoriaSolucionSelectUltimaCalificacionPorId"))
            {
                this.BaseDeDatos.AddInParameter(command, "idSolucion", DbType.Guid, idSolucion);
        ...
        public void Crear(CalificacionAuditoria auditoria)
        {
            if (auditoria != null)
            {
                ...
                {
                    using (DbCommand command =
                        this.BaseDeDatos.GetStoredProcCommand("uspAuditoriaSolucionInsert"))
                    {
                        this.BaseDeDatos.AddInParameter(command, "idSolucion", DbType.Guid, itemAuditoria.IdSolucion);
        ...
        {
            using (DbCommand command = this.BaseDeDatos.GetStoredProcCommand("uspAuditoriaSolucionUpdate"))
            {
                this.BaseDeDatos.AddInParameter(command, "idSolucion", DbType.Guid, auditoria.IdSolucion);
        ...
        {
            using (DbCommand command =
                this.BaseDeDatos.GetStoredProcCommand("uspAuditoriaSolucionDeletePorId"))
            {
                this.BaseDeDatos.AddInParameter(command, "idSolucion", DbType.Guid, idSolucion);
                this.BaseDeDatos.ExecuteNonQuery(command);
            }
